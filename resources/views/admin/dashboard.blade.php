    ...
    <form method="GET" action="{{ route('admin.dashboard') }}" style="display:flex; gap:12px; align-items:flex-end;">
        <div>
            ...
            <input type="date" name="date" value="{{ $date }}" onchange="this.form.submit()"
                style="width:160px; border:1.5px solid #93C5FD; border-radius:8px; padding:8px 12px; font-size:13px; background:white; color:#0F172A; font-family:'DM Sans',sans-serif;">
        </div>
    </form>
</div>
...
<div style="display:grid; grid-template-columns:1.6fr 1fr; gap:20px; margin-bottom:24px;">
    ...
    <div style="background:#fff; border:1.5px solid #93C5FD; border-radius:12px; padding:22px 24px;">
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:20px;">
            ...
            <span style="font-size:11px; color:#1E40AF; font-family:'DM Sans',sans-serif; background:#DBEAFE; padding:3px 10px; border-radius:20px;">Attendance Count</span>
        </div>
        ...
        <div style="display:flex; align-items:flex-end; gap:10px; height:100px;">
            ...
            <div style="flex:1; display:flex; flex-direction:column; align-items:center; gap:6px;">
                ...
                <span style="font-size:9px; color:{{ $w['isCurrent'] ? '#1E40AF' : '#64748B' }}; font-family:'DM Sans',sans-serif; text-align:center; line-height:1.2; font-weight:{{ $w['isCurrent'] ? '600' : '400' }};">
                    {{ $w['label'] }}
                </span>
            </div>
            @endforeach
    ...
    <div style="background:#fff; border:1.5px solid #93C5FD; border-radius:12px; padding:22px 24px;">
        ...
        <div style="margin-bottom:12px;">
            <div style="display:flex; justify-content:space-between; margin-bottom:4px;">
                <span style="font-size:12px; color:#0F172A; font-family:'DM Sans',sans-serif;">{{ $church }}</span>
                <span style="font-size:12px; color:#1E40AF; font-weight:600; font-family:'DM Sans',sans-serif;">
...
<div style="display:grid; grid-template-columns:1fr 320px; gap:20px; margin-bottom:24px;">
    ...
    <div style="background:#fff; border:1.5px solid #93C5FD; border-radius:12px; overflow:hidden;">
        <div style="padding:18px 24px; border-bottom:1px solid #E2E8F0; display:flex; justify-content:space-between; align-items:center;">
            ...
            <span style="display:inline-block; padding:2px 12px; border-radius:20px; font-size:11px; background:#DBEAFE; color:#1E40AF; font-family:'DM Sans',sans-serif; font-weight:500;">
        ...
        <div style="max-height:360px; overflow-y:auto;">
            ...
            <div style="display:flex; align-items:center; gap:14px; padding:11px 24px; border-bottom:1px solid #F1F5F9; transition:background 0.15s;"
                ...
                <div style="flex:1; min-width:0;">
                    ...
                    <p style="margin:0; font-size:11px; color:#64748B;">{{ $a->email }}</p>
                </div>
                @if($ch)
                <span style="display:inline-block; padding:2px 8px; border-radius:20px; font-size:10px; font-weight:500; white-space:nowrap; background:{{ $badgeStyle['bg'] }}; color:{{ $badgeStyle['text'] }};">
                    {{ $ch }}
                </span>
                @endif
